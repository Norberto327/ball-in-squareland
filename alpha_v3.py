...
## NOTES TO SELF + TO DO #####
# - Fix high jump glitch (clips 1px on the side, then jumps 150%) (Maybe feature?)
#
...
pygame.display.set_caption("Movement alpha_v3") # Window name here
...
class Ball:
    def __init__(self):
        ...
        self.level = 0
        self.levelLocation = [0, 1]
Ball = Ball()

######### Platforms #######################################

geometry_lvl0 =[ [(100, 100, 100), (300, 490, 100, 50)   ],
...
geometry_lvl1 = [
                [(16, 32, 64), (50, 490, 200, 50)],
                [(64, 32, 16), (400, 400, 200, 9)]
                ]

levels_geometry = [geometry_lvl0, geometry_lvl1]
loop = 0
levelMap = [
            ["#", 0, 1, "#"],
...
while True:
 ...
 ### Movement ######################################
    ...
    if Ball.x < Ball.radius: # To the left
        if levelMap[Ball.levelLocation[0]][Ball.levelLocation[1]-1] != "#":
            Ball.x = Window.x - Ball.radius
            Ball.levelLocation[1] -= 1
            Ball.level = levelMap[Ball.levelLocation[0]][Ball.levelLocation[1]]
#             Ball.x = Window.x - Ball.radius
        else:
            Ball.Vx *= 0
            Ball.x = Ball.radius
...
#             Ball.x = Ball.radius
    ...
    for x in range(len(levels_geometry[Ball.level])):
        cola, colb, colc, cold = levels_geometry[Ball.level][x][1]
    ...
    for x in range(len(levels_geometry[Ball.level])):
        pygame.draw.rect(window, levels_geometry[Ball.level][x][0], levels_geometry[Ball.level][x][1])
